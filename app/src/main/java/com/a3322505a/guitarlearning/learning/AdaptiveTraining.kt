...
object AdaptiveTraining {
    ...
    private fun overloaded(samples: List<AssessmentSample>): Boolean =
    ...
    fun transition(s: LearnerState, now: Long): LearnerState {
        ...
        view.responses.filter { it.unit.startsWith("position:") || it.unit.startsWith("mapping:") }.groupBy { it.unit }.forEach { (key, responses) ->
