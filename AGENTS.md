...
- 页面框架改 `learning/LearningPageFrame.kt`，普通内容改 `LearningPages.kt`，训练分派/共享控件改 `TrainingScreen.kt`，题型排版改 `TrainingLayouts.kt`。`LearningApp.kt` 负责状态连接、导航、生命周期与文件选择器。不要重新把 ViewModel 或 LearnerState 传进显示组件。
