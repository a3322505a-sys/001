...
- 当前应用版本：**2.0.0-alpha17（versionCode 34）**，定义见 [app/build.gradle.kts](app/build.gradle.kts)。
...
- 指板训练分为低把位 0–4 品、中把位 5–8 品、全指板 0–12 品三个入口；区域进度继续按原两点节点证据计算。
- 设置 → 外观提供清爽青白、暖纸森林、午夜蓝、石墨紫；主题保存后立即生效。已掌握、可学习、未解锁、需复习与规划中采用一致的颜色、符号和文字。
- 指板训练只显示低／中／全三个区域与开始／继续；4–5题已学池摸底后混练、补教并引入满足先修的新点。点区域立即进入音位题；其他练习及低／中／全区域分别保留暂停题，回到对应入口时恢复。其他题型专项保持原有语义。
- 进阶应用提供四个和弦形态示例，设置可选颜色框、手指编号或音名；形态示范、发音及逐弦任务共用数据，O/X 和横按按实际发音处理。
- 训练页宽屏上方按题目/谱面/操作与说明/反馈分栏，下方保留完整固定指板；短谱按内容宽度排版，窄屏或大字回流，溢出只在信息区滚动。O/X 紧邻弦枕并对齐琴弦；普通题答对自动前进，答错纠正后手动下一题；创作题完成后保留回听和手动下一题。训练页横屏沉浸，退出恢复系统栏。
