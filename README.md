...
- P2/P3 训练页按文字、指板（含谱面）、和弦分派布局：纯选项题集中显示；指板按顶栏测量后的空间分配，短窗口滚动；和弦图与操作并排，小窗口或大字时回流。普通谱面与短谱共用内容宽度滚动策略，原生文字和谱面几何随辅助字号成套缩放。普通题答对自动前进，答错纠正后手动下一题，区域第12题纠正后自然结束；创作题完成后保留回听和手动下一题。训练页横屏沉浸，退出恢复系统栏。
