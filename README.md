...
R7 的可用范围明确为 C 大调、A 自然小调，C/A 根音下四类三和弦；音程识别覆盖 0–12 半音，听辨覆盖同度、大二度、小三度、大三度、纯四度、纯五度和八度。后续调性、完整节奏与实琴识别不在本批完成声明中。R7 交付过程中执行环境离线，代码检查可继续走 GitHub CI；正式签名包的交付状态见 PR 与执行记录，不把未签名 CI 包当作可覆盖安装包。
