...
alpha18 施工：界面减负、双向近期熟练度、诊断与动态回退、能力树聚合、音名/唱名/级数混合，以及显示后 400ms 首播。复用同一 Room 快照；旧条件不明记录保留且不计作新独立样本。代码仍在 feature 分支，尚未通过构建/CI，也未交付 APK；P6 等待指板试用及适配范围确认。详见 [分 P 施工记录](docs/p-plan-alpha18.md)。
